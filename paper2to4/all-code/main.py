...
def main():
    ...
    if  args.evaluate:                                                       #    (*)
        ...
        pretrained_dict = torch.load('E://GSM//hmdb//hmdb-5-rgb-new1.pth')   #    (!)
        model_dict = model.state_dict()
...
def validate(val_loader, model, criterion, epoch, log=None, tf_writer=None):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            top1.update(prec1.item(), input.size(0))
            top5.update(prec5.item(), input.size(0))
            #record_pred.append(output.cpu().numpy())                    (**)
            #record_target.append(target.cpu().numpy())                  (**)
            # measure elapsed time
            batch_time.update(time.time() - end)
            end = time.time()
            ...
            if i % args.print_freq == 0:
                ...
                if log is not None:
                    log.write(output + '\n')
                    log.flush()
    #with open('E://GSM//check//hmdb-5-rgb-new1.pickle', 'wb') as f:                               (**)
    #    pickle.dump(record_pred, f)                                                               (**)
    #with open('E://GSM//check//hmdb-5-rgb-new1-target.pickle', 'wb') as f1:                       (**)
