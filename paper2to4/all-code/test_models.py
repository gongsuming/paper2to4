...
# may contain splits
parser.add_argument('--weights', type=str, default="E://GSM//hmdb//hmdb-5-rgb-new1.pth")            # （**）
...
# for true test
parser.add_argument('--test_list', type=str, default="E://GSM//hmdb51_rgb_val_split_1.txt")          # （****）
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        for n_seg, (_, (data, label)), net, modality in zip(test_segments_list, data_label_pairs, net_list, modality_list):
            ...
            this_label = label
            record_pred.append(rst[1])                         #   (%)
            record_target.append(label.cpu().numpy())          #   (%)
...
with open('E://GSM//TSM_i3//hmdb-5-rgb-new1.pickle', 'wb') as f:                    #   (%)
    pickle.dump(record_pred, f)                                                     #   (%)
with open('E://GSM//TSM_i3//hmdb-5-rgb-new1-target.pickle', 'wb') as f1:            #   (%)
    pickle.dump(record_target, f1)                                                  #   (%)
...
print('Class Accuracy {:.05f}%'.format(np.mean(cls_acc) * 100))
