...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        if self.new_fc is None:
            ...
            if hasattr(self.new_fc, 'weight'):#hasattr(object, name)判断一个对象里面是否有name属性或者name方法
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            self.base_model = getattr(torchvision.models, base_model)(True if self.pretrain == 'imagenet' else False)#getattr() 函数用于返回一个对象属性值。
            if self.is_shift:
            ...
            if self.zhang5:
                print("try zhang5...")
                from ops.zhang5 import make_shuffle_c_t
                make_shuffle_c_t(self.base_model, self.num_segments)
